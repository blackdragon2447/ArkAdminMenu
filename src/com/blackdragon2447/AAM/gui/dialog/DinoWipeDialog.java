...
public class DinoWipeDialog extends JFrame {
	...
	JLabel DestroyDinoLabel = new JLabel("destroywilddinos");
	...
	public DinoWipeDialog() {
		...
		GridBagConstraints gbc_DestroyDinoLabel = new GridBagConstraints();
		gbc_DestroyDinoLabel.gridwidth = 2;
		gbc_DestroyDinoLabel.insets = new Insets(0, 0, 5, 5);
		gbc_DestroyDinoLabel.gridx = 1;
		gbc_DestroyDinoLabel.gridy = 7;
		contentPane.add(DestroyDinoLabel, gbc_DestroyDinoLabel);
		...
		public void run() {
			synchronized (commonObject) {
				...
				while(!exit) {
					...
					sendMessage("wiping dinos in 10 min, let an admin know in discord if you need more time");
					...
					if(exit) continue bodyLoop;
					sendMessage("wiping dinos in 5 min, let an admin know in discord if you need more time");
					if(paused) {
						try {
							commonObject.wait();
						} catch (InterruptedException e) {
					...
					if(exit) continue bodyLoop;
					sendMessage("wiping dinos in 3 min, let an admin know in discord if you need more time");
					if(paused) {
						try {
							commonObject.wait();
						} catch (InterruptedException e) {
					...
					Wait3Label.setForeground(DestroyDinoLabel.getForeground());
					
					if (!Reference.MultipleServer) {
						try {
							RconHandler.command("destroywilddinos");
						} catch (IOException | AuthenticationException e) {
							Main.logger.LogDebug(e.toString(), Level.SEVERE);
						}
					} else {
						try {
							RconHandler.MultipleCommand("destroywilddinos");
						} catch (IOException | AuthenticationException e) {
							Main.logger.LogDebug(e.toString(), Level.SEVERE);
						}
					}
